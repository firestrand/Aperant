...
export function GitHubIntegrationSection({
  ...
  const badge = envConfig.githubEnabled ? (
    <StatusBadge status="success" label={t('projectSections.memory.enabled')} />
  ) : null;
        ...
        <div className="rounded-lg border border-info/30 bg-info/5 p-3 mb-4">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-foreground">{t('projectSections.github.projectSpecificConfiguration')}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {t('projectSections.github.projectSpecificConfigurationPrefix')} <span className="font-semibold text-foreground">{projectName}</span>. {t('projectSections.github.projectSpecificConfigurationSuffix')}
              </p>
            </div>
          </div>
        </div>
        ...
        <>
          ...
          {envConfig.githubAuthMethod === 'oauth' && envConfig.githubToken && !showOAuthFlow ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('projectSections.github.authentication')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg border border-success/30 bg-success/5">
                ...
                <span className="text-sm text-foreground">{t('projectSections.github.authenticatedViaOAuth')}</span>
              </div>
            </div>
          ) : showOAuthFlow ? (
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('projectSections.github.createTokenWithRepoScope')}{' '}<code className="px-1 bg-muted rounded">repo</code>{' '}{t('projectSections.github.scopeFrom')}{' '}
                <a
          ...
          {envConfig.githubToken && envConfig.githubRepo && (
            <ConnectionStatus
              ...
              title={t('projectSections.github.connectionStatus')}
              successMessage={t('projectSections.github.connectedToRepo', { repo: gitHubConnectionStatus?.repoFullName })}
              errorMessage={gitHubConnectionStatus?.error || t('projectSections.github.notConnected')}
