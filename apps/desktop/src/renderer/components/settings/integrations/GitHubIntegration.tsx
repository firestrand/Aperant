  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setBranchesError(result.error || t('settings:projectSections.github.failedToLoadBranches'));
      }
    } catch (err) {
      debugLog('fetchBranches: Exception:', err);
      setBranchesError(err instanceof Error ? err.message : t('settings:projectSections.github.failedToLoadBranches'));
    } finally {
  ...
  const fetchUserRepos = async () => {
    ...
    try {
      ...
      if (result.success && result.data?.repos) {
        ...
        setReposError(result.error || t('settings:projectSections.github.failedToLoadRepositories'));
      }
    } catch (err) {
      debugLog('Error fetching repos:', err);
      setReposError(err instanceof Error ? err.message : t('settings:projectSections.github.failedToLoadRepositories'));
    } finally {
              ...
              <div className="rounded-lg border border-success/30 bg-success/10 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-success">{t('settings:projectSections.github.connectedViaCli')}</p>
                      {oauthUsername && (
                        <p className="text-xs text-success/80 flex items-center gap-1 mt-0.5">
                          <User className="h-3 w-3" />
                          {t('settings:projectSections.github.authenticatedAs', { username: oauthUsername })}
                        </p>
                      )}
                    </div>
                  </div>
                  <Button
                    ...
                    className="text-xs"
                  >
                    {t('settings:projectSections.github.useDifferentToken')}
                  </Button>
                </div>
              </div>
          ...
          {authMode === 'manual' && (
            <>
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('settings:projectSections.github.createTokenWithRepoScope')}{' '}<code className="px-1 bg-muted rounded">repo</code>{' '}{t('settings:projectSections.github.scopeFrom')}{' '}
                  <a
...
function RepositoryDropdown({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        >
          {isLoading ? (
            ...
            <span className="text-muted-foreground">{t('projectSections.github.selectRepository')}</span>
          )}
            ...
            <div className="max-h-48 overflow-y-auto">
              {filteredRepos.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('projectSections.github.noMatchingRepositories') : t('projectSections.github.noRepositoriesFound')}
                </div>
              ) : (
      ...
      {selectedRepo && (
        <p className="text-xs text-muted-foreground">
          {t('projectSections.github.selectedRepository')} <code className="px-1 bg-muted rounded">{selectedRepo}</code>
        </p>
      )}
    </div>
  );
...
function RepositoryInput({ value, onChange }: RepositoryInputProps) {
  ...
  return (
    <div className="space-y-2">
      <Label className="text-sm font-medium text-foreground">{t('projectSections.github.repository')}</Label>
      <p className="text-xs text-muted-foreground">
        {t('projectSections.github.repositoryFormat')} <code className="px-1 bg-muted rounded">owner/repo</code> {t('projectSections.github.repositoryExample')}
      </p>
      <Input
