...
function finding(id: string, severity: SeverityGroup): GitLabMRReviewFinding {
  return {
    id,
    severity,
    category: 'quality',
...
function grouped(findings: GitLabMRReviewFinding[]): Record<SeverityGroup, GitLabMRReviewFinding[]> {
  return {
...
describe('GitLab useFindingSelection', () => {
  it('drops disputed selections when selecting all active findings', () => {
    ...
    expect(idsFrom(onSelectionChange)).toEqual(['active-1']);
  });

  it('drops disputed selections when selecting important findings', () => {
    ...
    expect(idsFrom(onSelectionChange)).toEqual(['critical-1']);
  });
});
