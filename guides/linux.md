...
```bash
flatpak install --user apps/desktop/dist/Aperant-*.flatpak
```

### Running from Flatpak

```bash
flatpak run com.aperant.app
```
...
```bash
# Make executable
chmod +x Aperant-*-linux-x86_64.AppImage

# Run
./Aperant-*-linux-x86_64.AppImage
```
...
```bash
sudo dpkg -i Aperant-*-linux-amd64.deb
```
...
ldd ./Aperant-*-linux-x86_64.AppImage

# Try running with debug output
./Aperant-*-linux-x86_64.AppImage --verbose
```
